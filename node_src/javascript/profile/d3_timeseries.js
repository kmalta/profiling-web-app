...
function(d3,d3tip)
{
  ...
  var default_colors = ["#e26060","#a6cee3","#ff7f00","#b2df8a","#1f78b4","#fdbf6f","#33a02c","#cab2d6","#6a3d9a","#fb9a99","#e31a1c","#ffff99","#b15928"];
  ...
  var timeseries = function()
  {
    ...
    var svg,container,serieContainer,annotationsContainer,drawerContainer,tip,mousevline;
        ...
        xscale.setformat = xscale.tickFormat(d3.format("d"))

        //default tool tip function
        var _tipFunction = function(epoch_num,series) {
    ...
    function createLines(serie){
      ...
      var line = d3.svg.line()
            ...
            serie.find = function(epoch_num){
                    var bisect = d3.bisector(fk(aes.x)).left;
                    var i = bisect(serie.data,epoch_num)-1
                    if(i==-1)
                        return null
                    //look to far after serie is defined
                    if(i==serie.data.length-1 && serie.data.length>1 &&
                         Number(epoch_num)-Number(serie.data[i][aes.x])>Number(serie.data[i][aes.x])-Number(serie.data[i-1][aes.x]))
                         return null
                    return serie.data[i]
            }
        ...
        function updatefocusRing(xepoch){
            ...
            if(xepoch==null){
                ...
                s=s.data(series.map(function(s,i){
                        return { x:xepoch,item:s.find(xepoch),
                                    aes:s.aes,
        ...
        function updateTip(xepoch){
            if(xepoch==null){
                ...
                var s=series.map(function(s,i){
                        return {item:s.find(xepoch),
                                    aes:s.aes,options:s.options}
                    })

                tooltipDiv.style('opacity',0.9)
                                    .style('left',(margin.left+5+xscale(xepoch))+'px')
                                    .style('top',"0px")
                                    .html(_tipFunction(xepoch,s))
    ...
    var chart = function(elem)
    {
      ...
      yscale.range([height - margin.top - margin.bottom - drawerHeight - drawerTopMargin,0])
            .domain([d3.min(series.map(fk('ymin'))),
                    d3.max(series.map(fk('ymax')))])
            .nice()



      xscale.range([0,width-margin.left-margin.right])
            .domain([d3.min(series.map(fk('xmin'))),
                    d3.max(series.map(fk('xmax')))])
            .nice()
